...
mod tests {
    ...
    fn test_semantic_analyzer_with_package_resolution() {
        ...
        assert!(result.is_ok(), "Semantic analysis should not fail for external imports: {:?}", result);
    }
    ...
    fn test_transpiler_with_package_resolution() {
        ...
        assert!(js_output.contains("import"), "Should contain import statements");
        assert!(js_output.contains("express"), "Should contain express import");
        assert!(js_output.contains("lodash") || js_output.contains("debounce"), "Should contain lodash imports");
