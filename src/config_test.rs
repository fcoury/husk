...
"#;
        ...
        assert_eq!(config.package.description, Some("A comprehensive Husk application".to_string()));
        assert_eq!(config.package.author, Some("Jane Developer <jane@example.com>".to_string()));
        assert_eq!(config.package.license, Some("Apache-2.0".to_string()));
        assert_eq!(config.package.repository, Some("https://github.com/jane/comprehensive-app".to_string()));
        assert_eq!(config.package.homepage, Some("https://comprehensive-app.dev".to_string()));
        ...
        match config.dependencies.get("lodash").unwrap() {
            DependencySpec::Detailed { version: Some(v), optional, .. } => {
                assert_eq!(v, "^4.17.21");
        ...
        match config.dependencies.get("experimental").unwrap() {
            DependencySpec::Detailed { git: Some(g), tag: Some(t), .. } => {
    ...
    fn test_generate_package_json_from_config() {
        let config = HuskConfig {
            ...
            dependencies: {
                ...
                deps.insert("lodash".to_string(), DependencySpec::Detailed {
                ...
                deps.insert("local-lib".to_string(), DependencySpec::Detailed {
                    version: None,
        ...
        assert_eq!(parsed["repository"]["url"], "https://github.com/test/package");
...
"#;
        ...
        assert_eq!(found_dir.canonicalize().unwrap(), project_dir.canonicalize().unwrap());
    }
    ...
    fn test_get_all_dependencies() {
        ...
        config.dependencies.insert("prod-dep".to_string(), DependencySpec::Simple("^1.0.0".to_string()));
        config.dev_dependencies.insert("dev-dep".to_string(), DependencySpec::Simple("^2.0.0".to_string()));
