...
mod tests {
    use crate::{
        interpreter::InterpreterVisitor, lexer::Lexer, parser::Parser, semantic::SemanticVisitor,
    ...
    fn test_try_operator_semantic_analysis() {
        ...
        assert!(
            result.is_ok(),
            "Semantic analysis should succeed for Result types"
        );
    }
    ...
    fn test_try_operator_transpilation() {
        ...
        assert!(
            js_code.contains("__result"),
            "Should contain try operator logic"
        );
        assert!(
            js_code.contains("type === 'Err'"),
    ...
    fn test_try_operator_interpreter_err() {
        ...
        assert!(
            error_msg.contains("error propagation"),
            "Should mention error propagation"
        );
    }
    ...
    fn test_try_operator_invalid_type() {
        ...
        let error_msg = result.unwrap_err().to_string();
        assert!(
            error_msg.contains("Result types"),
            "Should mention Result types requirement"
        );
    }
