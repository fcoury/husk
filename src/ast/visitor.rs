use crate::{
    parser::{EnumVariant, Expr, ExternItem, Operator, Stmt, UnaryOp, UseItems, UsePath},
    span::Span,
};
    ...
    fn visit_expr(&mut self, expr: &Expr) -> std::result::Result<T, Self::Error> {
        match expr {
            ...
            Expr::Range(start, end, inclusive, span) => {
                self.visit_range(start.as_deref(), end.as_deref(), *inclusive, span)
            }
            ...
            Expr::CompoundAssign(left, op, right, span) => {
                self.visit_compound_assign(left, op, right, span)
            }
            ...
            Expr::MemberAccess(object, field, span) => {
                self.visit_member_access(object, field, span)
            }
            ...
            Expr::Closure(params, ret_type, body, span) => {
                self.visit_closure(params, ret_type, body, span)
            }
            Expr::MethodCall(object, method, args, span) => {
                self.visit_method_call(object, method, args, span)
            }
            Expr::Cast(expr, target_type, span) => self.visit_cast(expr, target_type, span),
            Expr::StructPattern(variant, fields, span) => {
