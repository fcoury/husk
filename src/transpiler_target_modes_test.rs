...
mod tests {
    ...
    use crate::config::{HuskConfig, BuildConfig, TargetConfig};
    ...
    fn test_transpile_node_esm_target() {
        ...
        let config = create_config_for_target(
            "node",
            "esm",
            "node-esm"
        );
    ...
    fn test_transpile_node_cjs_target() {
        ...
        assert!(js.contains("const { readFile } = require('fs');") || 
    ...
    fn test_transpile_umd_format() {
        ...
        assert!(js.contains("(function (root, factory)") || 
                js.contains("typeof exports === 'object'"));
