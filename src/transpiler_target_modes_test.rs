...
mod tests {
    ...
    fn transpile_with_target(code: &str, target: &str) -> Result<String> {
        use crate::transpiler::{JsTranspiler, ModuleFormat};
        use crate::{Lexer, Parser, SemanticVisitor};
        ...
        // Create transpiler with the appropriate module format
        let module_format = match target {
            "node-cjs" => ModuleFormat::CommonJS,
            _ => ModuleFormat::ESModule,
        };

        let mut transpiler = JsTranspiler::with_module_format(module_format);
    ...
    fn test_transpile_node_cjs_target() {
        ...
        let js = transpile_with_target(code, "node-cjs").unwrap();

        // Should generate CommonJS syntax
        assert!(js.contains("const fs = require('fs')")); // File I/O functions use require
        assert!(js.contains("exports.main = main")); // Public functions are exported with CommonJS syntax
        assert!(js.contains("exports.helper = helper"));
    ...
    fn test_invalid_target_config() {
        ...
        let result = transpile_with_target(code, "node-esm");
